    ...
    public function listSettings()
    {
        ...
        if (!$isAdmin) {
            $settings = [];
              ...
              $spTable = TableRegistry::get('system_setting');
           $rstdata = $spTable->find('all', [ 'conditions' => ['setting' => 'avaiable_disk_space']])->first();
            
            if (empty($rstdata)) {
                $rstdata = $spTable->newEmptyEntity();
            }
           $df = disk_free_space(__DIR__);
           $rstdata->setting  = "avaiable_disk_space";
           $rstdata->value = $this->filesize_formatted($df);
