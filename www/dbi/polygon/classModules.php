...
class GetInfo
{
  ...
  function geocode($address){
    ...
    if($resp['status']=='OK'){
       ...
       $bounds = isset($resp['results'][0]['geometry']['bounds']) ? $resp['results'][0]['geometry']['bounds'] : array();
        ...
        if($lati && $longi && $formatted_address){
            ...
            $data_arr["bounds"]  = json_encode($bounds);
           // $data_arr["country"] =  trim(end(explode(",", $formatted_address)));          
            return $data_arr;
